...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_inputs = np.dot(X, self.weights_input_to_hidden) # signals into hidden layer # (n_hidden, )
            ...
            output_error_term = error # scalar
            hidden_error = output_error_term * self.weights_hidden_to_output # (n_hidden, 1)
            hidden_error_term = (hidden_outputs * (1 - hidden_outputs) * hidden_error.T) # (1, n_hidden)

            # Weight step (input to hidden)
            delta_weights_i_h += X[:,None] * hidden_error_term # (n_feature, n_hidden)
            delta_weights_h_o += (output_error_term * hidden_outputs)[:,None] # (n_hidden, )
    ...
    def run(self, features):
        ...
        final_inputs = np.dot(hidden_outputs, self.weights_hidden_to_output) # signals into final output layer
        # 为什么最后不用sigmoid处理？ 输出层的激活函数根据需要选择，该项目是回归问题，输出值不在[0,1]内，不能用sigmoid
